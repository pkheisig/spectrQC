build_reference_matrix <- function(
  ...
  subsample_n = 5000
    ...
    get_ellipse <- function(mean, sigma, level = 0.95, n = 100, scale = 1.0) {
        ...
        ellipse_x <- a * cos(theta); ellipse_y <- b * sin(theta)
    ...
    fit_gmm_populations <- function(data, max_k = 5, min_prop = 0.05) {
        ...
        sigmas <- lapply(1:fit$G, function(k) {
            if (fit$modelName %in% c("EII", "VII")) diag(fit$parameters$variance$sigmasq[k], nrow = 2)
            else fit$parameters$variance$sigma[, , k]
        })
    ...
    get_sample_type <- function(filename, patterns, default) {
        for (type in names(patterns)) {
            ...
            for (p in pats) if (grepl(p, filename, fixed = FALSE, ignore.case = TRUE)) return(list(type = type, pattern = p))
        }
        list(type = default, pattern = "default")
    }

    select_bead_population <- function(gmm_result) {
        if (length(gmm_result$main_populations) == 0) return(NULL)
    ...
    create_merged_gate <- function(gmm_result, populations, level, scale = 1.0, clip_x = Inf, clip_y = Inf) {
        ...
        if (length(populations) == 1) ell <- get_ellipse(gmm_result$means[, populations], gmm_result$sigmas[[populations]], level, scale = scale)
        else {
        ...
        ell[, x := pmax(0, pmin(x, clip_x))]; ell[, y := pmax(0, pmin(y, clip_y))]; return(ell)
    }
    ...
    for (fcs_file in fcs_files) {
        sn_ext <- basename(fcs_file); sn <- tools::file_path_sans_ext(sn_ext)
        ...
        ff <- flowCore::read.FCS(fcs_file, transformation = FALSE, truncate_max_range = FALSE); pd <- flowCore::pData(flowCore::parameters(ff)); raw_data <- flowCore::exprs(ff)
        fsc <- pd$name[grepl("^FSC", pd$name) & grepl("-A$", pd$name)][1]; ssc <- pd$name[grepl("^SSC", pd$name) & grepl("-A$", pd$name)][1]
        
        data_raw_scatter <- raw_data[, c(fsc, ssc)]; fsc_max <- quantile(data_raw_scatter[, 1], 1 - outlier_percentile, na.rm = TRUE); ssc_max <- quantile(data_raw_scatter[, 2], 1 - outlier_percentile, na.rm = TRUE)
        ...
        } else debris_threshold <- 0

        if (!is.null(subsample_n) && nrow(data_filtered) > subsample_n) data_fit <- data_filtered[sample(nrow(data_filtered), subsample_n), ]
        else data_fit <- data_filtered

        gmm_result <- fit_gmm_populations(data_fit, max_k = max_clusters, min_prop = min_cluster_proportion)
        if (is.null(gmm_result)) next
        if (sample_info$type == "beads") { selected_pops <- select_bead_population(gmm_result)$selected; gate_level <- gate_contour_beads }
        else { selected_pops <- select_cell_populations(gmm_result, debris_threshold, min(fsc_max, ssc_max))$selected; gate_level <- gate_contour_cells
        ...
        peak_vals <- gated_data[, peak_channel]; vals_log <- log10(pmax(peak_vals, 1))
        if (sample_info$type %in% c("unstained", "cells")) vals_above <- vals_log else {
            d_full <- density(vals_log, n = 512); mid_idx <- which(d_full$x > histogram_min_x_log & d_full$x < quantile(vals_log, 0.98))
        ...
        if (dir == "right") { lq <- 0.5; uq <- 0.5 + pct } else if (dir == "left") { lq <- 0.5 - pct; uq <- 0.5 } else { lq <- 0.5 - pct / 2; uq <- 0.5 + pct / 2 }
        gate_min <- 10^quantile(vals_above, max(0, lq)); gate_max <- 10^quantile(vals_above, min(1, uq))
        ...
        p1 <- ggplot2::ggplot(dt2d, ggplot2::aes(x, y, fill = fill)) + ggplot2::geom_tile(width = diff(x_breaks)[1], height = diff(y_breaks)[1]) + ggplot2::scale_fill_gradientn(colors = c("#0000FF", "#00FFFF", "#00FF00", "#FFFF00", "#FF0000"), guide = "none") + ggplot2::geom_path(data = final_gate, ggplot2::aes(x, y), inherit.aes = FALSE, color = "red", linewidth = 1) + ggplot2::labs(title = paste0(sn, " - FSC/SSC"), subtitle = paste0(round(100 * nrow(gated_data) / nrow(raw_data), 1), "% gated"), x = pd$desc[pd$name == fsc], y = pd$desc[pd$name == ssc]) + ggplot2::theme_minimal() + ggplot2::theme(legend.position = "none", panel.grid = element_blank(), panel.background = element_rect(fill = "white", color = NA)) + ggplot2::coord_cartesian(xlim = c(0, max(fsc_max, ssc_max)*1.05), ylim = c(0, max(fsc_max, ssc_max)*1.05))
        ggplot2::ggsave(file.path(out_path, "fsc_ssc", paste0(sn, "_fsc_ssc.png")), p1, width = 5, height = 5, dpi = 300)

        p2 <- ggplot2::ggplot(data.table::data.table(x = vals_log), ggplot2::aes(x)) + ggplot2::geom_density(fill = "grey80", color = "grey40") + ggplot2::geom_vline(xintercept = log10(gate_min), color = "red", linewidth = 1) + ggplot2::geom_vline(xintercept = log10(gate_max), color = "red", linewidth = 1) + ggplot2::annotate("rect", xmin = log10(gate_min), xmax = log10(gate_max), ymin = -Inf, ymax = Inf, alpha = 0.15, fill = "red") + ggplot2::labs(title = paste0(sn, " - ", peak_channel), subtitle = paste0(round(100 * nrow(final_gated_data) / nrow(gated_data), 1), "% gated"), x = paste0("log10(", peak_channel, ")")) + ggplot2::theme_minimal() + ggplot2::theme(legend.position = "none")
        ...
        # Filter for positive log values to avoid power transformation issues
        log_mat_pos <- pmax(log_mat, 0)
        max_y <- max(3, ceiling(max(log_mat_pos)))
        breaks <- seq(0, max_y, length.out = 1001)
        bin_mid <- (breaks[-1] + breaks[-length(breaks)]) / 2
        counts_mat <- vapply(seq_len(ncol(log_mat_pos)), function(j) as.numeric(hist(log_mat_pos[, j], breaks = breaks, plot = FALSE)$counts), numeric(length(bin_mid)))
        ...
        dt_c[, `:=`(y = bin_mid[bin_idx]^1.1, fill = log10(count + 1))]
        dt_c <- dt_c[count >= 1]
        ...
        vlines <- which(diff(det_info$laser_nm) != 0) + 0.5 # Need to update det_info to include laser_nm

        p3 <- ggplot2::ggplot(dt_c, ggplot2::aes(ch_idx, y, fill = fill)) +
            ggplot2::geom_tile(width = 0.7, height = (breaks[2]-breaks[1]) * 3) +
            ggplot2::scale_fill_gradientn(colors = colorRampPalette(c("#0000FF", "#00FFFF", "#00FF00", "#FFFF00", "#FF0000"))(100), 
                                         limits = c(min(dt_c$fill, na.rm=TRUE), quantile(dt_c$fill, 0.99, na.rm=TRUE)), 
                                         oob = scales::squish) +
            ggplot2::scale_x_continuous(breaks = seq_along(detector_names), labels = detector_labels) +
            ggplot2::scale_y_continuous(limits = c(0, (max_y + 0.5)^1.1), breaks = (0:max_y)^1.1, labels = sapply(0:max_y, function(x) bquote(10^.(x)))) +
            ggplot2::coord_cartesian(expand = FALSE) +
            ggplot2::labs(title = paste0(sn, " - Spectrum"), x = NULL, y = "Intensity") +
            ggplot2::theme_minimal() +
            ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 90, hjust = 1, vjust = 0.5, size = 6), legend.position = "none", panel.background = element_rect(fill="white", color=NA))
    ...
    results_dt <- data.table::rbindlist(results_list); spectra_list <- results_dt$spectrum; names(spectra_list) <- results_dt$fluorophore
    if (!is.null(af_data_raw)) spectra_list[["AF"]] <- af_data_raw / max(af_data_raw)
    M <- do.call(rbind, spectra_list); colnames(M) <- detector_names
    M_df <- as.data.frame(M); M_df$file <- rownames(M_df); data.table::fwrite(M_df[, c("file", colnames(M))], "reference_matrix.csv")
    message("Reference matrix (", nrow(M), " markers) saved."); return(M)
}
