...
- **What to look for**: Low values (light colors) are ideal. 
- **Warning signs**: High values (dark red) indicate that two fluorophores have very similar spectra. This means you will lose sensitivity when trying to detect dim signals on cells that are also positive for the "spreading" fluorophore.
